...
$folio = $_GET['folio'];
    ...
    $pdf->MultiCell(0,5,utf8_decode(strtoupper("TICKET DE ABASTECIMIENTO")),0,'C',false);
    $pdf->SetFont('Arial','',9);
    ...
    if($medicamentos_abastecidos -> num_rows > 0)
    {
        ...
        while($med_abastecido = mysqli_fetch_array($medicamentos_abastecidos))
        {
            ...
            $total_venta += $med['Precio']*$med_abastecido['Cantidad_medicamento'];
    ...
    $pdf->Cell(18,5,utf8_decode(""),0,0,'C');
    $pdf->Cell(26,5,utf8_decode("TOTAL PAGADO"),0,0,'C');
    $pdf->Cell(32,5,utf8_decode("$ ".$total_final." MXN"),0,0,'C');

    $pdf->Ln(5);

    $pdf->Cell(18,5,utf8_decode(""),0,0,'C');
    $pdf->Cell(26,5,utf8_decode("TOTAL A VENDER"),0,0,'C');
    $pdf->Cell(32,5,utf8_decode("$ ".$total_venta." MXN"),0,0,'C');

    $pdf->Ln(5);

    $pdf->Cell(18,5,utf8_decode(""),0,0,'C');
    $pdf->Cell(26,5,utf8_decode("GANANCIAS EST"),0,0,'C');
    $pdf->Cell(32,5,utf8_decode("$ ".$total_venta-$total_final." MXN"),0,0,'C');
