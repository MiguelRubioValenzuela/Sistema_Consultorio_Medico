...
if(isset($_GET['envia_curp']))
{
    ...
    if(isset($_GET['curp']))
    {
        $where = "SELECT * FROM ventas WHERE stat = 'Vendido' AND Fecha LIKE'%".$busqueda."%' ORDER BY Fecha DESC, Hora DESC; ";
    }
}
?>
            <div class="buscar2">  
...
if(isset($_GET['enviar_nombre']))
{
    ...
    {
        $where = "SELECT * FROM ventas WHERE stat = 'Vendido' AND Folio_ventas LIKE'%".$busqueda."%' ORDER BY Fecha DESC, Hora DESC; ";
    }
}
?>
        </div> 
        <div class="formulario">
            <h1>Busqueda de las ventas realizadas, avienta los resultados siguientes:</h1>
...
?>
    <div class="espacio">
        <table>
            <tr>
                <td style="background-color: cyan; font-size:24px; font-weight: bolder;">Folio: <?php echo $mostrar['Folio_ventas'];?></td>
                <td colspan="4" class="fecha">
                    Fecha compra: <?php echo $mostrar['Fecha'];?>   ----   Hora compra: <?php echo $mostrar['Hora'];?>.
                </td>
            </tr>
            <tr>
                <td class="pos" >Nombre de medicamento:</td>
                <td class="pos" >Precio de venta actual:</td>
                <td class="pos" >Cantidad vendida:</td>
                <td class="pos" >Precio cuando se vendio:</td>
                <td class="pos" >Total parcial:</td>
            </tr>
            <div class="scroll">
                <?php
                $folio = $mostrar['Folio_ventas'];
                $medicamento_abastecido = "SELECT * FROM medicamento_ventas WHERE Folio_venta = '$folio' AND stat = 'Vendido';";
            ...
            <tr>
                <td colspan="2" style="font-size:20px; font-weight: bolder;">Usuario que realizó la venta:</td>
